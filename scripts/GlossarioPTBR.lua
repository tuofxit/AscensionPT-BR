-- ================================================
-- GLOSSÁRIO OFICIAL - ASCENSION PT-BR
-- Versão: 1.0
--
-- Este arquivo é a única fonte de terminologia do projeto.
--
-- Sempre que um termo existir aqui,
-- utilize EXATAMENTE esta tradução.
--
-- Nunca invente outra tradução.
-- ================================================

GlossarioPTBR = {
    --------------------------------------------------
    -- INTERFACE
    --------------------------------------------------
    ["Ability"] = "Habilidade",
    ["Abilities"] = "Habilidades",
    ["Action Bar"] = "Barra de ações",
    ["Action Bars"] = "Barras de ações",
    ["Adventure"] = "Aventura",
    ["Activities"] = "Atividades",
    ["Appearance"] = "Aparência",
    ["Auction House"] = "Casa de Leilões",
    ["Bag"] = "Bolsa",
    ["Bank"] = "Banco",
    ["Calendar"] = "Calendário",
    ["Character"] = "Personagem",
    ["Character Info"] = "Informações do Personagem",
    ["Character Panel"] = "Painel do Personagem",
    ["Collections"] = "Coleções",
    ["Combat"] = "Combate",
    ["Currency"] = "Moeda",
    ["Equipment"] = "Equipamento",
    ["Game Menu"] = "Menu do Jogo",
    ["Guild"] = "Guild",
    ["Help"] = "Ajuda",
    ["Interface"] = "Interface",
    ["Inventory"] = "Inventário",
    ["Item"] = "Item",
    ["Items"] = "Itens",
    ["Journal"] = "Diário",
    ["Loot"] = "Saque",
    ["Mail"] = "Correio",
    ["Map"] = "Mapa",
    ["Merchant"] = "Comerciante",
    ["Quest"] = "Missão",
    ["Quests"] = "Missões",
    ["Quest Log"] = "Registro de Missões",
    ["Settings"] = "Configurações",
    ["Spell"] = "Feitiço",
    ["Spells"] = "Feitiços",
    ["Spellbook"] = "Grimório",
    ["Spellbook & Abilities"] = "Grimório e Habilidades",
    ["Talent"] = "Talento",
    ["Talents"] = "Talentos",
    ["Tooltip"] = "Dica",
    ["Trainer"] = "Treinador",
    ["Trade"] = "Comércio",

    --------------------------------------------------
    -- ATRIBUTOS
    --------------------------------------------------
    ["Strength"] = "Força",
    ["Agility"] = "Agilidade",
    ["Intellect"] = "Intelecto",
    ["Spirit"] = "Espírito",
    ["Stamina"] = "Vigor",
    ["Armor"] = "Armadura",
    ["Defense"] = "Defesa",
    ["Health"] = "Vida",
    ["Mana"] = "Mana",

    --------------------------------------------------
    -- ESTATÍSTICAS
    --------------------------------------------------
    ["Attack Power"] = "Poder de ataque",
    ["PvE Power"] = "Poder PvE",
    ["PvP Power"] = "Poder PvP",
    ["Hit Rating"] = "Índice de acerto",
    ["Critical Strike Rating"] = "Índice de acerto crítico",
    ["Crit Chance"] = "Chance crítica",
    ["Haste Rating"] = "Índice de aceleração",
    ["Expertise Rating"] = "Índice de perícia",
    ["Spell Power"] = "Poder dos feitiços",
    ["Spell Penetration"] = "Penetração de feitiços",
    ["Armor Penetration"] = "Penetração de armadura",
    ["Mana Regen"] = "Regeneração de mana",
    ["Health Regen"] = "Regeneração de vida",
    ["Speed"] = "Velocidade",
    ["Movement Speed"] = "Velocidade de movimento",
    ["Damage"] = "Dano",
    ["Melee"] = "Corpo a corpo",
    ["Ranged"] = "À distância",
    ["Off-Hand"] = "Mão secundária",
    ["Off-Hand Hit"] = "Acerto com a mão secundária",
    ["Dodge"] = "Esquiva",
    ["Parry"] = "Aparo",
    ["Block"] = "Bloqueio",
    ["Resilience"] = "Resiliência",

    --------------------------------------------------
    -- RESISTÊNCIAS
    --------------------------------------------------
    ["Arcane Resistance"] = "Resistência ao Arcano",
    ["Fire Resistance"] = "Resistência ao Fogo",
    ["Nature Resistance"] = "Resistência à Natureza",
    ["Frost Resistance"] = "Resistência ao Gelo",
    ["Shadow Resistance"] = "Resistência às Sombras",

    --------------------------------------------------
    -- ITENS
    --------------------------------------------------
    ["Item Level"] = "Nível do item",
    ["Required Level"] = "Requer nível",
    ["Requires Level"] = "Requer nível",
    ["Sell Price"] = "Preço de venda",
    ["Buy Price"] = "Preço de compra",
    ["Durability"] = "Durabilidade",
    ["Equip"] = "Equipar",
    ["Use"] = "Usar",
    ["Unique"] = "Único",
    ["Unique Equipped"] = "Único (Equipado)",
    ["Bind on Pickup"] = "Ligado ao pegar",
    ["Bind on Equip"] = "Ligado ao equipar",
    ["Soulbound"] = "Ligado",
    ["Vendor"] = "Comerciante",

    --------------------------------------------------
    -- EQUIPAMENTOS
    --------------------------------------------------
    ["Head"] = "Cabeça",
    ["Neck"] = "Pescoço",
    ["Shoulder"] = "Ombros",
    ["Back"] = "Costas",
    ["Chest"] = "Peito",
    ["Shirt"] = "Camisa",
    ["Tabard"] = "Tabardo",
    ["Wrist"] = "Punhos",
    ["Hands"] = "Mãos",
    ["Waist"] = "Cintura",
    ["Legs"] = "Pernas",
    ["Feet"] = "Pés",
    ["Finger"] = "Dedo",
    ["Trinket"] = "Berloque",
    ["Main Hand"] = "Mão principal",
    ["Off Hand"] = "Mão secundária",
    ["Two-Hand"] = "Duas mãos",
    ["One-Hand"] = "Uma mão",
    ["Ranged"] = "À distância",

    --------------------------------------------------
    -- TIPOS DE ARMADURA
    --------------------------------------------------
    ["Cloth"] = "Tecido",
    ["Leather"] = "Couro",
    ["Mail"] = "Malha",
    ["Plate"] = "Placas",
    ["Shield"] = "Escudo",

    --------------------------------------------------
    -- QUALIDADE DOS ITENS
    --------------------------------------------------
    ["Poor"] = "Poor",
    ["Common"] = "Common",
    ["Uncommon"] = "Uncommon",
    ["Rare"] = "Rare",
    ["Epic"] = "Epic",
    ["Legendary"] = "Legendary",
    ["Artifact"] = "Artifact",
    ["Heirloom"] = "Heirloom",

    --------------------------------------------------
    -- CLASSES
    --------------------------------------------------
    ["Warrior"] = "Guerreiro",
    ["Paladin"] = "Paladino",
    ["Hunter"] = "Caçador",
    ["Rogue"] = "Ladino",
    ["Priest"] = "Sacerdote",
    ["Death Knight"] = "Cavaleiro da Morte",
    ["Shaman"] = "Xamã",
    ["Mage"] = "Mago",
    ["Warlock"] = "Bruxo",
    ["Druid"] = "Druida",

    --------------------------------------------------
    -- PROFISSÕES
    --------------------------------------------------
    ["Alchemy"] = "Alquimia",
    ["Blacksmithing"] = "Ferraria",
    ["Enchanting"] = "Encantamento",
    ["Engineering"] = "Engenharia",
    ["Herbalism"] = "Herbalismo",
    ["Inscription"] = "Escrivania",
    ["Jewelcrafting"] = "Joalheria",
    ["Leatherworking"] = "Couraria",
    ["Mining"] = "Mineração",
    ["Skinning"] = "Esfolamento",
    ["Tailoring"] = "Alfaiataria",
    ["Fishing"] = "Pesca",
    ["Cooking"] = "Culinária",
    ["First Aid"] = "Primeiros Socorros",

    --------------------------------------------------
    -- BOTÕES
    --------------------------------------------------
    ["Accept"] = "Aceitar",
    ["Cancel"] = "Cancelar",
    ["Close"] = "Fechar",
    ["Complete"] = "Concluir",
    ["Continue"] = "Continuar",
    ["Create"] = "Criar",
    ["Delete"] = "Excluir",
    ["Equip"] = "Equipar",
    ["Exit"] = "Sair",
    ["Finish"] = "Finalizar",
    ["Learn"] = "Aprender",
    ["Next"] = "Próximo",
    ["No"] = "Não",
    ["Previous"] = "Anterior",
    ["Save"] = "Salvar",
    ["Search"] = "Procurar",
    ["Yes"] = "Sim",

    --------------------------------------------------
    -- MENSAGENS
    --------------------------------------------------
    ["Requires"] = "Requer",
    ["Unavailable"] = "Indisponível",
    ["Available"] = "Disponível",
    ["Completed"] = "Concluído",
    ["Incomplete"] = "Incompleto",
    ["Locked"] = "Bloqueado",
    ["Unlocked"] = "Desbloqueado",
    ["Enabled"] = "Ativado",
    ["Disabled"] = "Desativado",

    --------------------------------------------------
    -- TERMOS DO ASCENSION
    --------------------------------------------------
    ["Ascension"] = "Ascension",
    ["Path to Ascension"] = "Path to Ascension",
    ["Trial"] = "Trial",
    ["Trials"] = "Trials",
    ["Prestige"] = "Prestige",
    ["Nightmare"] = "Nightmare",
    ["Call Board"] = "Call Board",
    ["Worldforged"] = "Worldforged",
    ["World Boss"] = "World Boss",
    ["Raid"] = "Raid",
    ["Guild"] = "Guild",
}
